...
function createResultsWorkbook(
  ...
  const headers: string[] = ['N° Candidat', 'Nom', 'Prénom', 'Sexe', 'Établissement'];
  ...
  if (options.filterStatus && options.filterStatus !== 'all') {
    filteredStudents = filteredStudents.filter((s) => s.status === options.filterStatus);
  }
  ...
  switch (options.sortBy) {
    ...
    case 'school':
      filteredStudents.sort((a, b) => 
        a.schoolName.localeCompare(b.schoolName) || (a.rank ?? 9999) - (b.rank ?? 9999)
      );
      break;
  }
...
function createSchoolResultsWorkbook(
  ...
  const admitted = filteredStudents.filter((s) => s.status === 'admitted').length;
  const failed = filteredStudents.filter((s) => s.status === 'failed').length;
  const successRate = filteredStudents.length > 0 
...
function createSchoolStatsWorkbook(
  ...
  const sortedStats = [...schoolStats].sort((a, b) => b.successRate - a.successRate);
  ...
  rows.push([
    ...
    totals.admitted,
    totals.failed,
    totals.candidates > 0 ? Math.round((totals.admitted / totals.candidates) * 100) : 0,
    '-',
  ]);
...
function addPdfHeader(
  ...
  doc.text(`${examInfo.name} - Session ${examInfo.year}`, pageWidth / 2, 20, { align: 'center' });
  ...
  doc.text(`Seuil de réussite: ${examInfo.passingGrade}/${examInfo.maxGrade}`, 14, 44);
...
function createResultsPdf(
  ...
  const orientation = useScores && subjects.length > 4 ? 'landscape' : 'portrait';
  ...
  doc.setFontSize(10);
  doc.setFont('helvetica', 'bold');
  doc.text(`Total: ${filteredStudents.length} candidats | Admis: ${admitted} | Ajournés: ${failed}`, 14, finalY);
...
function createSchoolStatsPdf(
  ...
  const columns = ['Établissement', 'Candidats', 'Admis', 'Ajournés', 'Taux (%)', 'Moyenne'];
 ...
 */
async function downloadExcelTauri(workbook: XLSX.WorkBook, filename: string): Promise<void> {
...
export async function exportSchoolResultsToExcel(
  ...
  const sanitizedName = schoolName.replace(/[^a-zA-Z0-9À-ÿ\s]/g, '').replace(/\s+/g, '_');
...
function createRoomWorkbook(
  ...
  const headers = ['N°', 'Nom', 'Prénom', 'Établissement'];
...
function createRoomPdf(
  ...
  doc.setFontSize(14);
  doc.text(`Salle ${room.roomNumber}${room.roomName ? ` - ${room.roomName}` : ''}`, pageWidth / 2, 30, { align: 'center' });

  doc.setFontSize(10);
  doc.setFont('helvetica', 'normal');
  doc.text(`Effectif: ${room.students.length} / ${room.capacity} places`, 14, 42);
  doc.text(`Date: ${formatDate()}`, 14, 48);

  const columns = ['N°', 'Nom', 'Prénom', 'Établissement'];
